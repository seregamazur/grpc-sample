...
kustomize() {
    docker build --no-cache -f k8s/kotlin-k8s/client/Dockerfile -t grpc-java-client . \
    && docker build --no-cache -f k8s/kotlin-k8s/server/Dockerfile -t grpc-java-server . \
    && kubectl apply -k k8s/
}
